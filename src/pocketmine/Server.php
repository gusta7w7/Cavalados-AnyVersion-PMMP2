...
class Server
{
    ...
    public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath, $defaultLang = "unknown")
    {
        ...
        try {
            ...
            if (strpos(\pocketmine\VERSION, "unsupported") !== false && getenv("GITLAB_CI") === false) {
            ...
            if ($this->dserverConfig["enable"] && ($this->getAdvancedProperty("dserver.server-list", "") != "")) {
                $this->scheduler->scheduleRepeatingTask(new CallbackTask([
                    $this,
                    "updateDServerInfo"
    ...
    public function batchPackets(array $players, array $packets, $forceSync = false)
    {
        ...
        foreach ($packets as $p) {
            ...
            if ($p instanceof DataPacket || $p instanceof \pocketmine\network\protocol\p70\DataPacket) {
                if (!$p->isEncoded) {
                    $p->encode();
                }
    ...
    public function broadcastPacketsCallback($data, array $identifiers)
    {
        ...
        foreach ($identifiers as $i) {
            if (isset($this->players[$i])) {
                ...
                } elseif (AnyVersionManager::isProtocol($this->players[$i], "0.14")) {
    ...
    public function updatePlayerListData(/*UUID*/ $uuid, $entityId, $name, $skinId, $skinData, array $players = null)
    {
        ...
        $sendTo = $players === null ? $this->playerList : $players;
        foreach ($sendTo as $p) {
            $p->dataPacket($pk);
        }
    }
    ...
    public function sendRecipeList(Player $p)
    {
        ...
        } elseif (AnyVersionManager::isProtocol($p, "0.14")) {
