...
namespace pocketmine {
    ...
    function getReferenceCount($value, $includeCurrent = true)
    {
        ...
        if (\count($ret) >= 1 && \preg_match('/^.* refcount\\(([0-9]+)\\)\\{$/', \trim($ret[0]), $m) > 0) {
    ...
    function getTrace($start = 1, $trace = null)
    {
        ...
        for ($i = (int) $start; isset($trace[$i]); ++$i, ++$j) {
            ...
            $messages[] = "#$j " . (isset($trace[$i]["file"]) ? cleanPath($trace[$i]["file"]) : "") . "(" . ($trace[$i]["line"] ?? "") . "): " . (isset($trace[$i]["class"]) ? $trace[$i]["class"] . (($trace[$i]["type"] === "dynamic" || $trace[$i]["type"] === "->") ? "->" : "::") : "") . $trace[$i]["function"] . "(" . Utils::printable(\substr($params, 0, -2)) . ")";
        }
    ...
    $logger->info("Fechando outras threads");

    foreach (ThreadManager::getInstance()->getAll() as $id => $thread) {
        $logger->debug("Fechando " . (new \ReflectionClass($thread))->getShortName() . " thread");
        $thread->quit();
    }
    ...
    echo "O servidor foi desligado com sucesso..." . Terminal::$FORMAT_RESET . "\n";
